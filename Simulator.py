...
class Simulator(object):
    def __init__(self, fovy=78, resolution=(1280,720)):
        ...
        glTranslatef(0, -1.3*_SCALE, 0)
        
        
    def run(self):
        while self.running:
        
            # # Prevents video render backlog and RAM crash
            # if self.simulations % 50 and self.simulations != 0:
                # time.sleep(60)
                
            scenarioID = self.tm.newScenario()
            self.recorder.openNew(scenarioID)
            print('INFO: Simulating scenarioID: %s' % scenarioID)
